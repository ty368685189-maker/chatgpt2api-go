...
- 画图主页面大幅重写，新增提示词输入优化、画幅/清晰度选择改进、多图参考交互优化。
- 新增**提示词模板**页面 (`/templates`)，可保存和复用常用提示词。
- 图片结果展示区重构，支持更流畅的浏览和下载体验。
- 图片灯箱 (lightbox) 增强，支持更好的预览交互。
- 侧边栏优化，操作更直观。

**安全与稳定性修复**

- 修复路径穿越漏洞 (path traversal)，加强文件访问安全。
- 修复并发冲突问题，多任务同时执行时不再互相干扰。
- 修复 React 渲染问题，前端页面更稳定。

**账号池与监控**

- `pool-status` API 新增 `avg_latency_ms` 字段，可查看每个账号的生图平均耗时。
- 前端状态栏实时显示生图平均耗时（>1s 显示秒，否则显示毫秒）。
- 公告系统升级：内容改用 `content` 字段，支持回车直接换行；兼容旧版 `items` 数组格式。

**用户与权限系统**

- 新增**用户管理**页面 (`/users`)。
- 新增**邀请注册**系统，支持邀请码生成和管理。
- 登录/注册页面优化。

**上游传输修复**

- 修复 `curl-impersonate` 模式下 HTTP 代理 CONNECT 隧道响应被误解析的问题。
- 之前走 HTTP 代理时所有上游请求都会失败（`invalid character H`），现已修复。

**内存缓存优化**

- 引入内存缓存层，减少磁盘 JSON 文件频繁读写，提升高并发下的响应速度。

### 2026-06-14

- 新增 JSON 格式 `tool_calls` 兼容支持。
- 修复图片结果检测回退逻辑。

### 2026-06-13

- 首次上传 Go 源码和发布工作流。
- 发布包支持 Linux x86_64 / ARM64 / macOS x86_64。
- 捆绑 `curl-impersonate` 二进制。
