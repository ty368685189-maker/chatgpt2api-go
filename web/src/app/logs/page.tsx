    ...
    <section className="mt-4 space-y-5 sm:mt-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-1">
          <div className="text-xs font-semibold tracking-[0.18em] text-muted-foreground uppercase">Logs</div>
          <h1 className="text-2xl font-semibold tracking-tight">日志管理</h1>
        </div>
        <div className="flex flex-wrap gap-2">
          <Select value={type} onValueChange={setType}>
            <SelectTrigger className="h-10 w-[150px] rounded-xl border-border bg-card text-foreground hover:bg-secondary"><SelectValue /></SelectTrigger>
            <SelectContent>
          ...
          <Button onClick={() => void loadLogs()} disabled={isLoading} className="h-10 rounded-xl bg-primary px-4 text-primary-foreground hover:bg-primary/90">
        ...
        <CardContent className="p-0">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-4">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" className="h-8 rounded-lg px-3 text-muted-foreground hover:bg-secondary hover:text-foreground" onClick={() => void loadLogs()} disabled={isLoading}>
                <RefreshCw className={`size-4 ${isLoading ? "animate-spin" : ""}`} />
                刷新
              </Button>
              <button type="button" className="text-sm text-muted-foreground hover:text-foreground disabled:text-muted-foreground/30" onClick={() => setSelectedIds([])} disabled={selectedIds.length === 0 || isDeleting}>
                取消选择
              </button>
              <Button variant="outline" className="h-8 rounded-lg border-rose-250 bg-card px-3 text-rose-500 hover:bg-rose-500/10" onClick={() => setDeletingItems(items.filter((item) => selectedSet.has(item.id)))} disabled={selectedIds.length === 0 || isDeleting}>
            ...
            <Table className="min-w-[900px]">
              ...
              <TableBody>
                {currentRows.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id} className="text-foreground/80">
                      <TableCell>
                            ...
                            <div className="flex items-center gap-1.5">
                              {urls.slice(0, 3).map((url, imageIndex) => (
                                <button
                                  key={`${url}-${imageIndex}`}
                                  type="button"
                                  className="relative size-9 overflow-hidden rounded-lg border border-border bg-secondary"
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          <Button variant="ghost" className="h-8 rounded-lg px-3 text-muted-foreground hover:bg-secondary hover:text-foreground" onClick={() => openDetail(item)}>
                            查看详情
                          </Button>
                          <Button variant="ghost" className="h-8 rounded-lg px-3 text-rose-500 hover:bg-rose-500/10 hover:text-rose-600" onClick={() => setDeletingItems([item])}>
                            删除
                          </Button>
                        </div>
                      </TableCell>
          ...
          {!isLoading && items.length === 0 ? <div className="px-6 py-14 text-center text-sm text-muted-foreground">没有找到日志</div> : null}
        </CardContent>
      </Card>
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="flex h-[min(88vh,860px)] w-[min(92vw,920px)] flex-col overflow-hidden rounded-2xl p-0 border-border bg-card">
          <DialogHeader className="shrink-0 border-b border-border px-6 py-5">
            <DialogTitle>日志详情</DialogTitle>
          </DialogHeader>
          <div className="flex-1 overflow-y-auto px-6 py-5">
            <div className="space-y-4">
              <div className="grid gap-3 rounded-xl border border-border bg-secondary/35 p-4 text-sm text-muted-foreground md:grid-cols-2">
                {Object.entries(detailLog?.detail || {})
                  .filter(([key, value]) => key !== "urls" && typeof value !== "object")
                  .map(([key, value]) => (
                    <div key={key} className="flex items-start justify-between gap-4">
                      <span className="text-muted-foreground">{key}</span>
                      <span className="text-right font-medium break-all text-foreground">{String(value)}</span>
                    </div>
                  ))}
              </div>
              {detailUrls.length ? (
              ...
              <pre className="max-h-[72vh] overflow-auto rounded-xl border border-border bg-secondary/50 p-4 text-xs leading-6 text-foreground">
      ...
      <Dialog open={deletingItems.length > 0} onOpenChange={(open) => (!open ? setDeletingItems([]) : null)}>
        <DialogContent showCloseButton={false} className="rounded-2xl p-6 border-border bg-card">
