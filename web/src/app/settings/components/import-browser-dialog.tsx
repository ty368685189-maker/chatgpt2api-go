...
export function ImportBrowserDialog() {
  ...
  return (
    <Dialog open={browserOpen} onOpenChange={setBrowserOpen}>
      <DialogContent showCloseButton={false} className="max-h-[90vh] max-w-5xl rounded-2xl p-6 border-border bg-background">
        <DialogHeader className="gap-2">
          <DialogTitle className="text-foreground">选择要导入的账号</DialogTitle>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative min-w-[260px]">
            ...
            <Input
              ...
              className="h-10 rounded-xl border-border bg-background text-foreground pl-10"
            />
          </div>
          <div className="flex items-center gap-2">
            <Select value={pageSize} onValueChange={(value) => setPageSize(value as (typeof PAGE_SIZE_OPTIONS)[number])}>
              <SelectTrigger className="h-10 w-[120px] rounded-xl border-border bg-background text-foreground">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <Button
              variant="outline"
              className="h-10 rounded-xl border-border bg-background px-4 text-foreground/90"
        ...
        <div className="rounded-xl border border-border">
          <div className="flex items-center justify-between border-b border-border/50 px-4 py-3 text-sm text-muted-foreground">
          ...
          <div className="max-h-[420px] overflow-auto">
            {pagedFiles.length === 0 ? (
              <div className="flex items-center justify-center py-12 text-sm text-muted-foreground/80">没有匹配的远程账号</div>
            ) : (
                ...
                {pagedFiles.map((item) => (
                  <label key={item.name} className="flex cursor-pointer items-center gap-3 px-4 py-3 hover:bg-muted">
                    ...
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium text-foreground">{item.email || item.name}</div>
        ...
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-2 text-foreground">
            <Button
              variant="outline"
              className="h-9 rounded-xl border-border bg-background px-3"
